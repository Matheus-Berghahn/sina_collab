...
const TextoAnimadoItem = ({ texto, isRight }: { texto: string; isRight: boolean }) => {
  ...
  return (
    <div ref={ref} className="relative w-full tracking-[8px]">
      <motion.h2
  ...
  return (
    <div className="flex flex-col items-center space-y-16 w-full px-8 pt-32 overflow-x-hidden cursor-crosshair bg-black">
      {textos.map((texto, index) => (
        <TextoAnimadoItem key={index} texto={texto} isRight={index % 2 === 0} />
      ))}
    </div>
  );
}
